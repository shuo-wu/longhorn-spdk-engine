...
func (r *Replica) RebuildingSrcRangeShallowCopyStart(spdkClient *spdkclient.Client, snapshotName, dstRebuildingLvolAddress string, mismatchingClusterList []uint64) (err error) {
	...
	go func() {
		...
		for head < totalMismatchingClusterCount {
			...
			r.log.Infof(">>>>>>> Rebuilding Src Replica %s is starting range shallow copy snapshot %s(%s)(%s): mismatching clusters %+v", r.Name, snapshotName, snapSvcLvolAlias, snapSvcLvolUUID, mismatchingClusterList[head:tail])
			...
			r.log.Infof(">>>>>>> Rebuilding Src Replica %s finished range shallow copy snapshot %s(%s)(%s): mismatching clusters %+v", r.Name, snapshotName, snapSvcLvolAlias, snapSvcLvolUUID, mismatchingClusterList[head:tail])

			head = tail
			tail += lvolRangeShallowCopyLength
		}

		r.log.Infof("Rebuilding Src Replica finished all range shallow copy for snapshot %s(%s)(%s), mismatching clusters %+v", snapshotName, snapSvcLvolAlias, snapSvcLvolUUID, mismatchingClusterList)
	}()
...
func (r *Replica) rebuildingDstRangeShallowCopy(spdkClient *spdkclient.Client, srcReplicaServiceCli *client.SPDKClient, snapshotName, dstRebuildingLvolAddress string) (err error) {
	...
	dstSnapshotBdevLvol, err := spdkClient.BdevLvolGetByName(dstSnapshotLvolAlias, 0)
	if err != nil {
		return errors.Wrapf(err, "failed to get the dst snapshot lvol %s for rebuilding dst replica %v snapshot %s range shallow copy", dstSnapshotLvolAlias, r.Name, snapshotName)
	}
	r.log.Infof(">>>> Replica snapshot lvol %s has Parent %s and Children %+v before deletion for rebuilding dst replica %v snapshot %s range shallow copy", dstSnapshotLvolAlias, dstSnapshotBdevLvol.DriverSpecific.Lvol.BaseSnapshot, dstSnapshotBdevLvol.DriverSpecific.Lvol.Clones, r.Name, snapshotName)
	...
	if r.rebuildingDstCache.rebuildingLvol.Parent != "" {
		// Just in case, detach the parent of the rebuilding lvol is again.
		// As we mentioned above, spdk_tgt requires the rebuilding lvol being backed by a zeros bdev so that it can release clusters after unmapping the rebuilding lvol.
		r.log.Infof(">>>> Replica is detaching the rebuilding lvol %s from its parent %s for dst replica %v snapshot %s range shallow copy", r.rebuildingDstCache.rebuildingLvol.Alias, r.rebuildingDstCache.rebuildingLvol.Parent, r.Name, snapshotName)
		if _, err := spdkClient.BdevLvolDetachParent(r.rebuildingDstCache.rebuildingLvol.Alias); err != nil && !jsonrpc.IsJSONRPCRespErrorNoSuchDevice(err) {
			return errors.Wrapf(err, "failed to detach the rebuilding lvol %s from its parent for rebuilding dst replica %v snapshot %s range shallow copy", r.rebuildingDstCache.rebuildingLvol.Alias, r.Name, snapshotName)
		}
	}
